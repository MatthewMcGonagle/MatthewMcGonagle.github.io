...
# Maximum values for coordinate functions.
maxVals = [100, 100]

# Set up domain value arrays.
X = np.arange(maxVals[0])[np.newaxis, :]
X = np.full((maxVals[1], maxVals[0]), X, dtype = 'float32')

Y = np.arange(maxVals[1])[::-1, np.newaxis]
Y = np.full((maxVals[1], maxVals[0]), Y, dtype = 'float32')
...
plt.clf()
sns.heatmap(z, vmin = 0.0, vmax = 1.0, xticklabels = False, yticklabels = False)
...
for sizes in hidden_layer_sizes:
    ...
    plt.clf()
    sns.heatmap(z_predict, vmin = 0.0, vmax = 1.0, xticklabels = False, yticklabels = False)
